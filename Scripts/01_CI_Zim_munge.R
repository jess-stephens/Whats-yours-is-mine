...
df <-read_xlsx("Data/Master custom report template_FY21Q2_results.xlsx", sheet="Data")
  view(df)
  glimpse(df)
  names(df)
  ...
  df2 <- df %>% 
    ...
    mutate(operatingunit="Zimbabwe", orgunit=NA, orgunitid=NA) %>% #add column for OU (Zimbabwe), Orgunit (unknown) and Orgunitid (unknown)
    view
